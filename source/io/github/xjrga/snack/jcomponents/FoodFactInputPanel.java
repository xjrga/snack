...
public class FoodFactInputPanel {
	...
	public FoodFactInputPanel( TableFoodFactInput table ) {
		...
		FormLayout panelLayout = new FormLayout( "10px,300px:grow,5px,200px,5px,min,min,min,10px", // columns
	...
	private void fillWaterAmount() {
		...
		FormLayout lyo = new FormLayout( "p:grow,p:grow", // columns
				"fill:p:grow,p" // rows
		);
		...
		if ( optionValue == 0 ) {
			...
			if ( (s0 != null && s0.length() > 0) ) {
				...
				if ( checkNumber.pass() ) {
					...
					mcm = new MoistureContentModifier( initialWeight.doubleValue(), initialWaterWeight.doubleValue(),
	...
	private JPanel getWizardPanel() {
		...
		FormLayout layout = new FormLayout( "min:grow,min,min,min,min,min,min:grow", // columns
				"min" // rows
		);
	...
	private void fillFatAmount() {
		...
		if ( optionValue == 0 ) {
			...
			if ( (s0 != null && s0.length() > 0) && (s1 != null && s1.length() > 0) && (s2 != null && s2.length() > 0)
					&& (s3 != null && s3.length() > 0) ) {
