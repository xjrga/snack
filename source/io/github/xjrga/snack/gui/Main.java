...
public class Main {
	...
	private JPanel getEditorPanel() {
		...
		FormLayout lyo = new FormLayout( "min:grow", // columns
	...
	private JPanel getEditorTopPanel() {
		...
		JPanel pnl = new JPanel();
		FormLayout lyo = new FormLayout( "p:grow,4px,156px", // columns
				"fill:30px:grow" // rows
		);
		pnl.setLayout( lyo );
	...
	private JPanel getEditorDri( TableDri tblDri ) {
		...
		FormLayout lyo = new FormLayout( "p:grow", // columns
				"5dlu,min,5dlu,fill:min:grow" // rows
		);
		FormLayout lyoLifestage = new FormLayout( "5dlu,min:grow,5dlu,min", // columns
				"min" // rows
		);
				...
				if ( selectedMix != null ) {
					...
					try {
						...
						Future<List<List>> task = BackgroundExec
								.submit( new DriDiffTask( selectedMixId, lifestage.getLifeStageId() ) );
	...
	private JSplitPane getModelPanel() {
		...
		FormLayout lyoMainPanel = new FormLayout( "p:grow,right:p", // columns
				"fill:min:grow" // rows
		);
		FormLayout lyoRightPanel = new FormLayout( "160px", // columns
				"min,fill:min:grow" // rows
		);
		FormLayout lyoConstraintCountPanel = new FormLayout( "p,p", // columns
				"4dlu,min,min,min,min,4dlu" // rows
		);
		FormLayout lyoObjective = new FormLayout( "150px", // columns
		...
		JPanel pnl = new JPanel();
		FormLayout lyoPnl = new FormLayout( "4px,min:grow,4px,p,4px", // columns
	...
	private JPanel getFoodCategoriesPanel() {
		...
		FormLayout lyo = new FormLayout( "m:grow,m:grow", // columns
				"fill:min:grow" // rows
		);
		FormLayout lyo00 = new FormLayout( "min:grow", // columns
				"fill:min:grow" // rows
		);
		FormLayout lyo01 = new FormLayout( "min:grow", // columns
				"fill:min:grow,min" // rows
		);
		FormLayout lyo02 = new FormLayout( "min,min:grow", // columns
				"fill:16dlu,6dlu,fill:min:grow" // rows
		);
		FormLayout lyoButtons = new FormLayout( "min:grow,min,min,min,min,min:grow", // columns
				"min" // rows
		);
		FormLayout lyoButtons01 = new FormLayout( "min:grow,min,min,min:grow", // columns
				"fill:min:grow" // rows
		);
		FormLayout lyo03 = new FormLayout( "min:grow", // columns
				"fill:min:grow,min" // rows
		);
		...
		tblCategory.getSelectionModel().addListSelectionListener( ( ListSelectionEvent a ) -> {
			...
			try {
				...
				Future<List<List>> task = BackgroundExec
						.submit( new FoodsInCategoryTask( (category.getCategoryid()) ) );
	...
	private JPanel getFoodComparisonPanel() {
		...
		JLabel lblSearch = new JLabel( "Search: " );
		lblSearch.setToolTipText( "Search field input should be a valid regex expression (case insensitive match)" );
		FormLayout lyo = new FormLayout( "4px,391px:grow,4px,391px:grow,4px,678px:grow,4px", // columns
		...
		FormLayout lyo03 = new FormLayout( "min,min:grow", // columns
	...
	private JPanel getFoodQuantityConstraint() {
		...
		JPanel pnl = new JPanel();
		FormLayout panelLayout = new FormLayout( "p,p,p:grow,p", // columns
				"p,p,fill:p:grow,min" // rows
		);
		pnl.setLayout( panelLayout );
		tblFoodQuantityConstraint = new TableFoodQuantityConstraint();
		JScrollPane spTable = new JScrollPane( tblFoodQuantityConstraint );
		Integer width = TableColumnWidth.Food.getWidth() + TableColumnWidth.Nutrient.getWidth() + 21 + 90
	...
	private JPanel getFoodList() {
		...
		JPanel pnl = new JPanel();
		FormLayout paneLayout = new FormLayout( "4px,min:grow,4px,min,4px", // columns
		...
		FormLayout buttonPanelLayout = new FormLayout( "min:grow,min,min,min,min,min:grow", // columns
				"min" // rows
		);
		...
		mniImportMixModel.addActionListener( ( ActionEvent evt ) -> {
			...
			if ( returnVal == JFileChooser.APPROVE_OPTION ) {
				...
				try {
					...
					LifeStageDO find = LifestageFinder.find( (( SpinnerListModel ) spnLifestage.getModel()).getList(),
							lifeStageId );
	...
	private JPanel getMixComparisonPanel() {
		...
		lblSearch.setToolTipText( "Search field input should be a valid regex expression (case insensitive match)" );
		// create layouts
		FormLayout lyo00 = new FormLayout( "4px,391px:grow,4px,391px:grow,4px,678px:grow,4px", // columns
				"4px,min,fill:min:grow,4px" // rows
		);
		FormLayout lyo01 = new FormLayout( "min,min:grow", // columns
	...
	private JPanel getMixFoodPanel() {
		...
		JPanel pnl = new JPanel();
		FormLayout lyo = new FormLayout( "min:grow,min,min:grow", // columns
				"fill:min:grow" // rows
		);
		pnl.setLayout( lyo );
		JPanel pnlButtons = new JPanel();
		FormLayout lyoButtons = new FormLayout( "min", // columns
		...
		scr_food.setBorder( new TitledBorder( "Available Food" ) );
	...
	private JPanel getNoSolutionPanel() {
		...
		FormLayout lyo = new FormLayout( "pref:grow", // columns
				"fill:pref:grow" // rows
		);
		StringBuilder sb = new StringBuilder();
	...
	private JPanel getNutrientSearchPanel() {
		...
		FormLayout lyo = new FormLayout( "4dlu:grow,500dlu,4dlu:grow", // columns
				"4dlu,min,4dlu,fill:min:grow,4dlu" // rows
		);
		FormLayout lyo01 = new FormLayout( "4dlu,p,p,4dlu,p,p:grow,4dlu", // columns
	...
	private JPanel getNutrientRatioPanel() {
		...
		JPanel pnl = new JPanel();
		FormLayout lyo = new FormLayout( "p:grow,90px,p,p,370px:grow", // columns
				"p,p,fill:p:grow,min" // rows
		);
		pnl.setLayout( lyo );
		tblNutrientRatio = new TableNutrientRatioConstraint();
		JScrollPane scr = new JScrollPane( tblNutrientRatio );
		Integer width = TableColumnWidth.Nutrient.getWidth() + TableColumnWidth.Nutrient.getWidth() + 90 + 21 + 90
				+ TableColumnWidth.Scrollbar.getWidth();
	...
	private Boolean checkFoodRatioConstraint() {
		...
		if ( flag_listFoodA && flag_listNutrientA && flag_quantityA && flag_listFoodB && flag_listNutrientB
				&& flag_quantityB ) {
	...
	private void convertMixToFood() {
		...
		int optionValue = Message.showOptionDialogOkCancel( inputs,
				"Add Mix As Food Item - Would you like to save specified values?" );
		...
		if ( optionValue == 0 ) {
			...
			if ( completed ) {
				...
				try {
					...
					Future<List<List>> task = BackgroundExec
							.submit( new NamedMixFoodSortedByNameTask( selectedMixId ) );
					List<List> foods = task.get();
					tblMixFood.reload( foods );
	...
	private void addCategory() {
		...
		JComponent[] inputs = {
				new JLabel( "What is your new category name?" ), txtInput
		};
	...
	private void renameCategory() {
		...
		if ( !tblCategory.isSelectionEmpty() ) {
			...
			JTextField input = new JTextField();
			JComponent[] inputs = {
					new JLabel( "What is your new category name?" ), input
			};
			...
			if ( optionValue == 0 ) {
				...
				if ( categoryname != null && categoryname.length() > 0 ) {
					...
					try {
						...
						Future<Boolean> task = BackgroundExec
								.submit( new UpdateCategoryTask( category.getCategoryid(), categoryname ) );
						Boolean completed = task.get();
		...
		if ( !tblCategory.isSelectionEmpty() ) {
			...
			if ( !tblCategoryAllFoods.isSelectionEmpty() ) {
				...
				try {
					...
					Future<Integer> task0 = BackgroundExec
							.submit( new FoodCategoryCountTask( category.getCategoryid(), food.getFoodid() ) );
					Integer count = task0.get();

					if ( count == 0 ) {

						Future<Boolean> task1 = BackgroundExec
								.submit( new InsertFoodCategoryLinkTask( category.getCategoryid(), food.getFoodid() ) );
	...
	private void removeFoodFromCategory() {
		...
		if ( !tblCategory.isSelectionEmpty() ) {
			...
			try {
				...
				Future<Boolean> task0 = BackgroundExec
						.submit( new DeleteCategoryLinkTask( category.getCategoryid(), food.getFoodid() ) );
	...
	private void addDerivativeFoodToDatabase() {
		...
		if ( !tblFoodFacts.isSelectionEmpty() ) {
			...
			int optionValue = Message.showOptionDialogOkCancel( inputs,
					"Update New Food Item - Would you like to save specified values?" );
		...
		if ( !tblFoodFacts.isSelectionEmpty() ) {
			...
			if ( optionValue == 0 ) {
				...
				try {
					...
					Future<Boolean> task = BackgroundExec
							.submit( new UpdateFoodTask( food.getId_food_id(), food.getId_food_name() ) );
					Boolean completed = task.get();
				...
				try {
					...
					Future<Boolean> task = BackgroundExec
							.submit( new DenormalizeFoodFactsTask( food.getId_food_id() ) );
					Boolean completed = task.get();
	...
	private void executeMergeFoodQuantityConstraintTasks(
		...
		try {
			...
			Future<Boolean> task0 = BackgroundExec.submit( new MergeFoodQuantityConstraintTask( mix.getMixid(),
					foodDO.getFoodId(), nutrientDO.getNutr_no(), relationshipDO.getRelationshipid(), b ) );
			...
			addLogEntry( selectedMixName, "Add", "Food Quantity Constraint", sb.toString(), selectedMixId,
					foodDO.getFoodId(), nutrientDO.getNutr_no(), "", "", relationshipDO.getRelationshipid(), b, null );
	...
	private void deleteFoodQuantityConstraint() {
		...
		if ( !tblFoodQuantityConstraint.isSelectionEmpty() ) {
			...
			try {
				...
				Future<Boolean> task0 = BackgroundExec
						.submit( new DeleteFoodQuantityConstraintTask( mixid, foodid, nutrientid, relationshipid ) );
				Boolean completed = task0.get();
				...
				addLogEntry( selectedMixName, "Delete", "Food Quantity Constraint", sb.toString(), selectedMixId,
						row.getFoodid(), row.getNutrientid(), "", "", row.getRelationshipid(), row.getB(), null );
				Future<List<List>> task1 = BackgroundExec.submit( new FoodQuantityConstraintsTask( mixid ) );
				List<List> constraints = task1.get();
				tblFoodQuantityConstraint.reload( constraints );
				setConstraintCounts();
		...
		if ( checkFoodRatioConstraint() ) {
			...
			if ( numberCheck.pass() ) {
				...
				try {
					...
					Future<Boolean> task0 = BackgroundExec.submit( new MergeFoodRatioConstraintTask( selectedMixId,
							foodDOA.getFoodId(), nutrientDOA.getNutr_no(), foodDOB.getFoodId(),
							nutrientDOB.getNutr_no(), relationshipDO.getRelationshipid(), a, b ) );
					Boolean completed = task0.get();
					...
					addLogEntry( selectedMixName, "Add", "Nutrient Ratio Constraint", sb.toString(), selectedMixId,
							foodDOA.getFoodId(), nutrientDOA.getNutr_no(), foodDOB.getFoodId(),
							nutrientDOB.getNutr_no(), relationshipDO.getRelationshipid(), a, b );
	...
	private void deleteFoodRatioConstraint() {
		...
		if ( !tblFoodRatioConstraint.isSelectionEmpty() ) {
			...
			try {
				...
				Future<Boolean> task0 = BackgroundExec.submit( new DeleteFoodRatioConstraintTask( mixid, foodidA,
						nutrientidA, foodidB, nutrientidB, relationshipid ) );
				Boolean completed = task0.get();
				...
				sb.append( " / " );
				sb.append( (new DecimalFormat( "###0.0" )).format( row.getB() ) );
				addLogEntry( selectedMixName, "Delete", "Food Ratio Constraint", sb.toString(), selectedMixId,
						row.getFoodida(), row.getNutrientida(), row.getFoodidb(), row.getNutrientidb(),
						row.getRelationshipid(), row.getA(), row.getB() );
		...
		if ( checkNumber.pass() ) {
			...
			for ( Row row : rows ) {
				...
				try {
					...
					Future<Boolean> task0 = BackgroundExec
							.submit( new InsertAndCalculateFoodPortionTask( mixid, row.getMealid(), foodid, pcti ) );
					Boolean complete = task0.get();
		...
		if ( !tblMealPortions.isSelectionEmpty() ) {
			...
			if ( optionValue == 0 ) {
				...
				try {
					...
					String foodid = row.getFoodid();
					Future<Boolean> task0 = BackgroundExec.submit( new UpdateFoodPortionActualWeightTask( mixid, mealid,
							foodid, new BigDecimal( txtInput.getText() ) ) );
					Boolean complete = task0.get();
	...
	private void addMeal() {
		...
		JPanel pnl = new JPanel();
		FormLayout lyo = new FormLayout( "28dlu,34dlu,100dlu", // columns
		...
		if ( optionValue == 0 ) {
			...
			if ( checkNumber.pass() ) {
				...
				try {
					...
					Future<Integer> task = BackgroundExec
							.submit( new CreateMealTask( selectedMixId, mealname, mealorder ) );
			...
			if ( optionValue == 0 ) {
				...
				if ( checkNumber.pass() ) {
					...
					try {
						...
						Future<Boolean> task = BackgroundExec
								.submit( new UpdateMealTask( selectedMixId, mealid, mealname, mealorder ) );
						Boolean completed = task.get();
	...
	private void addMixFood() {
		...
		if ( node.isLeaf() ) {
			...
			try {
				...
				addLogEntry( selectedMixName, "Add", "Food", foodName, selectedMixId, foodId, "", "", "", null, null,
						null );
	...
	private void deleteMixFood() {
		...
		addLogEntry( selectedMixName, "Delete", "Food", selectedFoodName, selectedMixId, selectedFoodId, "", "", "",
				null, null, null );
		...
		if ( checkNutrientQuantityConstraint() ) {
			...
			if ( numberCheck.pass() ) {
				...
				try {
					...
					sb.append( " " );
					sb.append( (new DecimalFormat( "###0.0" )).format( b ) );
					addLogEntry( selectedMixName, "Add", "Nutrient Quantity Constraint", sb.toString(), selectedMixId,
							"", nutrientDO.getNutr_no(), "", "", relationshipDO.getRelationshipid(), b, null );
					Future<List<List>> task1 = BackgroundExec
							.submit( new NutrientQuantityConstraintsTask( selectedMixId ) );
					List<List> constraints = task1.get();
	...
	private void deleteNutrientQuantityConstraint() {
		...
		try {
			...
			Future<Boolean> task0 = BackgroundExec
					.submit( new DeleteNutrientQuantityConstraintTask( mixid, nutrientid, relationshipid ) );
			Boolean completed = task0.get();
			...
			addLogEntry( selectedMixName, "Delete", "Nutrient Quantity Constraint", sb.toString(), selectedMixId, "",
					row.getNutrientid(), "", "", row.getRelationshipid(), row.getB(), null );
		...
		if ( checkNutrientRatioConstraint() ) {
			...
			if ( numberCheck.pass() ) {
				...
				try {
					...
					Future<Boolean> task0 = BackgroundExec
							.submit( new MergeNutrientRatioConstraintTask( selectedMixId, nutrientDOA.getNutr_no(),
									nutrientDOB.getNutr_no(), relationshipDO.getRelationshipid(), A, B ) );
					Boolean completed = task0.get();
					...
					sb.append( (new DecimalFormat( "###0.0" )).format( B ) );
					addLogEntry( selectedMixName, "Add", "Nutrient Ratio Constraint", sb.toString(), selectedMixId, "",
							nutrientDOA.getNutr_no(), "", nutrientDOB.getNutr_no(), relationshipDO.getRelationshipid(),
							A, B );
					Future<List<List>> task1 = BackgroundExec
							.submit( new NutrientRatioConstraintsTask( selectedMixId ) );
					List<List> constraints = task1.get();
	...
	private void deleteNutrientRatioConstraint() {
		...
		try {
			...
			Future<Boolean> task0 = BackgroundExec
					.submit( new DeleteNutrientRatioConstraintTask( mixid, nutrientaid, nutrientbid, relationshipid ) );
			Boolean completed = task0.get();
			...
			sb.append( " / " );
			sb.append( (new DecimalFormat( "###0.0" )).format( row.getB() ) );
			addLogEntry( selectedMixName, "Delete", "Nutrient Ratio Constraint", sb.toString(), selectedMixId, "",
					row.getNutrientaid(), "", row.getNutrientbid(), row.getRelationshipid(), row.getA(), row.getB() );
		...
		try {
			...
			if ( solutionFound ) {
				...
				Runnable tasks = () -> {
					...
					(new UpdateMixAction( mixid, lifestageid, model, new BigDecimal( "1.0" ), new BigDecimal( "1.0" ),
							new BigDecimal( "1.0" ) )).execute();
	...
	private void addLogEntries( MixDO mix, String lifestage, Double score ) {
		...
		addLogEntry( mix.getName(), "Add", "Minimization Option", txtMinimization.getText(), mix.getMixid(), "", "", "",
				"", null, null, null );

		addLogEntry( mix.getName(), "Solve", "Total Nutrient Index Score", String.valueOf( score ), mix.getMixid(), "",
				"", "", "", null, null, null );
		...
		try {
			...
			lst.forEach( ( row ) -> {
				...
				try {
					...
					Future<LhsContainer> taskLhs = BackgroundExec
							.submit( new DriDevNutrientQuantityLhsTask( mixid, nutrientid ) );
					LhsContainer container = taskLhs.get();
					double[] coefficients = container.getCoefficients();
		...
		try {
			...
			lst.forEach( ( row ) -> {
				...
				try {
					...
					String eq = ( String ) row.get( "EQ" );
					Future<double[]> taskLhs = BackgroundExec
							.submit( new NutrientRatioLhsTask( mixid, nutrientid1, nutrientid2, relationshipid ) );
					double[] coefficients = taskLhs.get();
					lpmodel.addConstraint( coefficients, relationshipid, 0.0 );
					StringBuilder constraintName = new StringBuilder();
					constraintName.append( nutrient1 ).append( " / " ).append( nutrient2 ).append( " " ).append( eq )
		...
		try {
			...
			lst.forEach( ( row ) -> {
				...
				try {
					...
					String eq = ( String ) row.get( "EQ" );
					Future<double[]> taskLhs = BackgroundExec
							.submit( new FoodQuantityLhsTask( mixid, foodid, nutrientid, relationshipid ) );
					double[] coefficients = taskLhs.get();
					lpmodel.addConstraint( coefficients, relationshipid, b );
					StringBuilder constraintName = new StringBuilder();
					constraintName.append( food ).append( " AS " ).append( nutrient ).append( " " ).append( eq )
							.append( " " ).append( b );
		...
		try {
			...
			lst.forEach( ( row ) -> {
				...
				try {
					...
					lpmodel.addConstraint( coefficients, relationshipid, 0.0 );
					StringBuilder constraintName = new StringBuilder();
					constraintName.append( food1 ).append( " AS " ).append( nutrient1 ).append( " / " ).append( food2 )
							.append( " AS " ).append( nutrient2 ).append( " " ).append( eq ).append( " " ).append( a )
							.append( " / " ).append( b );
			...
			if ( !tblMeals.getSelectedValues().isEmpty() ) {
				...
				for ( TablePortion.Row row : rows ) {
					...
					try {
						...
						Future<Boolean> task0 = BackgroundExec
								.submit( new DeleteFoodPortionTask( mixid, mealid, foodid ) );
	...
	private void convertMicronutrient() {
		...
		int optionValue = Message.showOptionDialogOkCancel( inputs,
				"Percent of Daily Reference Intake (DRI) to Grams" );
		...
		if ( optionValue == 0 ) {
			...
			if ( s != null && s.length() > 0 ) {
				...
				try {
					...
					Future<List<DriDO>> task = BackgroundExec
							.submit( new LifestageDriTask( cmbNutrients.getSelectedItem().getNutr_no(),
	...
	private void calculateDigestibleCarbs() {
		...
		FormLayout lyo = new FormLayout( "min,30dlu", // columns
				"min,16dlu" // rows
		);
		JPanel pnl = new JPanel();
		pnl.setLayout( lyo );
	...
	private void createMix() {
		...
		JTextField txtInput = new JTextField();
		JComponent[] inputs = {
				new JLabel( "What would you like to call it?" ), txtInput
		};
		...
		try {
			...
			if ( completed ) {
				...
				tblNutrientQuantityConstraint.getStream().forEach( o -> {
					...
					addLogEntry( selectedMixName, "Delete", "Nutrient Quantity Constraint", sb.toString(),
							selectedMixId, "", ( String ) row.get( 1 ), "", "", ( Integer ) row.get( 2 ),
							( BigDecimal ) row.get( 5 ), null );
				...
				tblNutrientRatio.getStream().forEach( o -> {
					...
					addLogEntry( selectedMixName, "Delete", "Nutrient Ratio Constraint", sb.toString(), selectedMixId,
							...
							( BigDecimal ) row.get( 5 ), ( BigDecimal ) row.get( 8 ) );
	...
	private void renameMix() {
		...
		JTextField txtInput = new JTextField();
		JComponent[] inputs = {
				new JLabel( "What is your new mix name?" ), txtInput
		};
		...
		if ( optionValue == 0 ) {
			...
			if ( s != null && s.length() > 0 ) {
				...
				if ( checkNumber.pass() ) {
					...
					final AlphaLinolenicRequired n3_fatty_acid_recommendation = new AlphaLinolenicRequired(
							energyKcal );
	...
	private void setNutrientQuantityConstraintGuiValues() {
		...
		Integer relationshipid = row.getRelationshipid();
		BigDecimal q = row.getB();
		int nutrientindex = cmbNutrientQuantityNutrient
				.index( new NutrientDO( nutrientid, "", new BigDecimal( "-1" ) ) );
	...
	private void setNutrientRatioConstraintGuiValues() {
		...
		BigDecimal qB = row.getB();
		Integer relationshipid = row.getRelationshipid();
		int nutrientAindex = cmbNutrientRatioNutrientA
				.index( new NutrientDO( nutrientida, "", new BigDecimal( "-1" ) ) );
		int nutrientBindex = cmbNutrientRatioNutrientB
	...
	private void reload_tblmdl_nutrient_lookup() {
		...
		if ( checkNumber.pass() ) {
			...
			try {
				...
				Future<List<List>> task = BackgroundExec
						.submit( new NutrientContainingFoodsTask( nutrientDO.getNutr_no(), q ) );
				List<List> results = task.get();
				tblNutrientLookup.reload( results );
	...
	private void reloadResults( MixDO mix ) {
		...
		try {
			...
			Future<List<List>> task = BackgroundExec
					.submit( new DriDiffTask( mix.getMixid(), lifestage.getLifeStageId() ) );
			List<List> diff = task.get();
			tblDri.reload( diff );
	...
	private void createDriDeviationTniConstraint(
		...
		try {
			...
			Future<List<Map<String, Object>>> taskRhs = BackgroundExec
					.submit( new DriDevTniRhsTask( mixid, lifestageId ) );
			List<Map<String, Object>> lst = taskRhs.get();
			lst.forEach( ( row ) -> {
	...
	private void createDriDeviationSumDeficiencyConstraint(
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			Future<LhsContainer> taskLhs = BackgroundExec
					.submit( new DriDevSumDeficiencyLhsTask( mixid, lifestyleid ) );
			LhsContainer container = taskLhs.get();
			double[] coefficients = container.getCoefficients();
	...
	private JPanel getModelLogPanel() {
		...
		JPanel pnl = new JPanel();
		FormLayout lyo = new FormLayout( "p,p:grow", // columns
	...
	private void addLogEntry(
			String mix, String action, String type, String object, String mixid, String foodidA, String nutrientidA,
			String foodidB, String nutrientidB, Integer relationshipid, BigDecimal A, BigDecimal B ) {
