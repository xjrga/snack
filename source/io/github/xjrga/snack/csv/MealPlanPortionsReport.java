...
public class MealPlanPortionsReport {
	...
	enum Headers {
		MixId, MealId, FoodId, Meal, Food, Pct, ExpectedWt, ActualWt, MealOrder
	}
	...
	public void create( MixDO mixDataObject ) {
		...
		try ( FileWriter fileWriter = new FileWriter( "models/mealplanportions.csv" ) ) {
			...
			CSVFormat csvFormat = CSVFormat.DEFAULT.builder().setCommentMarker( '#' )
					.setHeaderComments( "Meal Plan Portions Report", comment.toString(), LocalDateTime.now() )
			...
			portions.forEach( row -> {
				...
				try {
					...
					csvPrinter.printRecord( mixid, mealid, foodid, meal, food, Utilities.strip( pct ),
							Utilities.strip( expectedwt ), Utilities.strip( actualwt ), mealorder );
