	...
	public void create() {
		...
		try ( FileWriter fileWriter = new FileWriter( "models/allfoods2.csv" ) ) {
			...
			CSVFormat csvFormat = CSVFormat.DEFAULT.builder().setCommentMarker( '#' )
					.setHeaderComments( "All Foods Report #2", comment.toString(), LocalDateTime.now() ).setHeader( rs )
					.get();
			...
			while ( rs.next() ) {
				...
				csvPrinter.printRecord( x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18,
