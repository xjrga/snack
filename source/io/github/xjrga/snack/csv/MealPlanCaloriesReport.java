...
public class MealPlanCaloriesReport {
	...
	enum Headers {
		Meal, Weight, eGross, eDigestible, eFat, eCarbs, eFatPlus, eProtein, eAlcohol
	}
	...
	public void create( MixDO mixDataObject ) {
		...
		try ( FileWriter fileWriter = new FileWriter( "models/mealplancalories.csv" ) ) {
			...
			CSVFormat csvFormat = CSVFormat.DEFAULT.builder().setCommentMarker( '#' )
					.setHeaderComments( "Meal Plan Calories Report", comment.toString(), LocalDateTime.now() )
			...
			try {
				...
				rows.forEach( row -> {
					...
					try {
						...
						csvPrinter.printRecord( meal, Utilities.strip( weight ), Utilities.strip( eGross ),
								Utilities.strip( eDigestible ), Utilities.strip( eFat ), Utilities.strip( eCarbs ),
								Utilities.strip( eFatPlus ), Utilities.strip( eProtein ), Utilities.strip( eAlcohol ) );
